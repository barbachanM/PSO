...
expData = pd.read_csv("RAWexpData_PSO.csv", index_col = 0)
...
horizontal_stack.to_csv("RAW_Amazon_validationDataSet_LDA.csv", header = True)
...
for i in range(pop_size):
    particle = {'RF':np.random.randint(20,500),'LDA': np.random.uniform(0.001,0.00001), 'SVM':np.random.uniform(0.1,7.0), 'LogReg':[np.random.uniform(0.1,7.0),np.random.randint(10,1000)], 'KNN':np.random.randint(10,100)} #hyperparam
...
for i in range(pop_size):
    ...
    p[3] = p[0] #best
    swarm.append(p)
out2 = open("RAW_PSO_AmazonBestParticle.txt", "w")

out2.write("Params:\n"+"c1: "+str(c1)+"\n"+"c2: "+str(c2)+"\n"+"w: "+str(w)+"\n"+"swarm size: "+str(pop_size)+"\n"+"iterations: "+str(iter_max)+"\n")
out = open("RAW_PSO_Amazon_trace.csv", "w")
