...
expData = pd.read_csv("ADASYN_PSOdata.csv")
...
horizontal_stack.to_csv("ADASYN_validationDataSet.csv", header = True)
...
for i in range(pop_size):
    ...
    p[3] = p[0] #best
    swarm.append(p)
out2 = open("VALIDATION_LDA_PSO_ADASYN_BestParticle.txt", "w")

out2.write("Params:\n"+"c1: "+str(c1)+"\n"+"c2: "+str(c2)+"\n"+"w: "+str(w)+"\n"+"swarm size: "+str(pop_size)+"\n"+"iterations: "+str(iter_max)+"\n")
out = open("VALIDATION_LDA_PSO_ADASYN_trace.csv", "w")
